...
### 7b. Frontend build (обязательно при изменениях в apps/web)
...
**Ожидание:** в конце вывода строка вида `✓ built in N.NNs`.  
**Артефакты:** `apps/web/dist/` — `index.html` + `assets/`.

> ⚠️ Frontend билдится на сервере из исходников.  
> `dist/` не коммитится в git — он всегда генерируется командой выше.  
> При каждом деплое, затрагивающем `apps/web/**`, выполнять **7b** после **7a**.
...
- содержит `index.html`  
- содержит подкаталог `assets/` с `.js` и `.css` файлами  
- дата файлов — соответствует времени последней сборки  
